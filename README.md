...
> **Figma 및 Framer 등 디자인 작업 시간을 스마트하게 기록하고 공유하는 크로스 플랫폼 앱**
>
> Figlog는 Figma, Framer, PowerPoint 등의 주요 작업 창이 활성화되어 있고 사용자가 실제로 조작하고 있는 시간을 자동으로 감지하여 기록하는 생산성 도구입니다. 
...
2. 다운로드된 `Figlog.dmg` 파일을 더블클릭하여 마운트한 뒤, 화면에 보이는 **Figlog** 아이콘을 우측의 **Applications(응용 프로그램)** 폴더로 드래그 앤 드롭하여 설치합니다.
...
* **다중 앱 완벽 트래킹 지원:** Figma뿐만 아니라 Framer(데스크톱/웹), PowerPoint 등 다양한 타겟 앱의 활성화 상태를 감지하여 개별적으로 시간을 기록합니다.
* **진짜 집중 시간만 기록:** 타겟 앱이 활성 창일 때 작동하며, 마우스 움직임과 키보드 입력이 멈추면 자동으로 '자리비움(Idle)' 상태로 전환하여 측정을 일시정지합니다.
* **실시간 작업 상태 표시:** 상단 트래킹 상태 메시지가 "Working in Figma", "Working in Framer"처럼 현재 활성화된 디자인 툴 이름을 직관적으로 보여줍니다.
...
* **실시간 상태 연동:** Firebase Firestore 연동을 통해 나의 상태(`Working in Figma`, `Working in Framer`, `자리비움`, `오프라인` 등)와 오늘 누적 집중 시간을 동료들과 실시간 공유합니다.
